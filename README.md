...
## Small-world structure, and why retrieval cares

```bash
Rscript -e 'source("R/smallworld.R"); print(sw_report())'
Rscript -e 'source("R/smallworld.R"); sw_augment(p = "auto")'
Rscript -e 'source("R/retrieve_graph.R"); print(sw_retrieval_gain())'
```

Graph-augmented retrieval seeds from vector search and then walks the graph
to reach what search missed — the other plans' version of a rule, the state
letter both implement. That walk has a small hop budget (2; three hops pulls
in most of a cluster), so whether it reaches another plan at all is a
property of the graph's topology rather than of the query.

Measured on the semantic layer (`implements`, `supersedes`, `similar_topic`,
`shortcut` — the hub stars `published_by`/`owned_by` are excluded, since one
